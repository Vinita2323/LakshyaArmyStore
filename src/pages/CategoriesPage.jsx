...
export default function CategoriesPage() {
  ...
  const renderCatIcon = (id, isActive) => {
    ...
    switch (id) {
      ...
      case 'uniform': imgSrc = catUniform; break;
      case 'toys': imgSrc = cat2; break; // teddy bear
      case 'footwear': imgSrc = catFootwear; break; // placeholder
      case 'watches-electronics': imgSrc = catWatches; break; // home appliances
  ...
  const getFilteredProducts = () => {
    ...
    switch (selectedCategory) {
      case 'for-you':
        ...
        break;
      case 'uniform':
        filtered = items.filter((p) => ['tee', 'pants', 'blouse', 'outfit'].includes(p.type));
        break;
      case 'footwear':
        filtered = items.filter((p) => ['shoes', 'sneakers', 'boots'].includes(p.type));
        break;
      case 'watches-electronics':
        filtered = items.filter((p) => ['watch', 'earbuds', 'powerbank', 'fan', 'headphones', 'smartwatch'].includes(p.type));
        break;
      case 'toys':
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-sm font-black text-white tracking-wide uppercase font-syne">
              Discover <span>Lakshya army store</span>
            </h1>
